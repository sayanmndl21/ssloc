...
cols = 60
winlist = []
# Initialize a dataframe of max. size == 4
log = logdata(4)
...
try:#don't want user warnings
    while True:
        ...
        if b:
            ...
            """ 
            predict the current sample result:
            """
            print("Sample Drone: \n    %s, label %s"% (dist_prediction_label(int(x1)), x1))
            log.insertdf(int(x1),str(datetime.datetime.now())[:-7])
            #print(x1)

            """
            output the new dataframe result: 
            """
            ...
            if True:#i > 9:
                ...
                output_label = int(output['Label'])
                print("DF Output: \n    %s, label [%s]\n" % (dist_prediction_label(output_label), output_label))
