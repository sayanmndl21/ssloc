...
for root, dirs, files in os.walk(sys.argv[1]):
    with open(root+'.csv', 'w',newline='') as f:
        fieldnames = ['ID','Height','Distance', 'LPCCOEFF','PERR','RCOEFF','PSD', 'Pfreq']
        ...
        for file in files:
            ...
            try:
                p, pf, b = psddetectionresults(data)
            except IndexError:
                pass
                b = False
            ...
            if b:
                ...
                if nfile_flag == 0:
                    ...
                    Distance1=stri[4:5]
                    if not np.isnan(a[1]):
                        thewriter.writerow({'ID':fnid,'Height':Height1,'Distance':Distance1, 'LPCCOEFF':a[1:],'PERR':e,'RCOEFF':k, 'PSD': p, 'Pfreq': pf})
                        fnid+=1
